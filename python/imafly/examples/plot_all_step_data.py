...
for i, filename in enumerate(filelist):
    ...
    plant_gain = data['plant_gain'][()]
    kj = plant_gain[:,0].mean()
    ...
    ki_list.append(fit[0])
    kj_list.append(kj)


kj_ki_pairs = list(zip(kj_list, ki_list))
kj_ki_pairs.sort()
kj_list = [x for x,y in kj_ki_pairs]
ki_list = [y for x,y in kj_ki_pairs]

ki_array = np.array(ki_list)
kj_array = np.array(kj_list)
kj_inv_array = 1.0/kj_array
km_fit = np.polyfit(kj_inv_array, ki_array, 1)
kj_inv_fit = np.linspace(kj_inv_array.min(), kj_inv_array.max(), 1000)
ki_fit = np.polyval(km_fit, kj_inv_fit)
kj_fit = 1.0/kj_inv_fit

km_median = np.median(ki_array*kj_array)
kj_median = np.linspace(kj_array.min(), kj_array.max(), 1000)
ki_median = km_median/kj_median

numer = ((1.0/kj_array)*ki_array).sum()
denom = ((1.0/kj_array)**2).sum()
km_lstsq = numer/denom

fig4, ax4 = plt.subplots(1,1)
ax4.plot(kj_inv_array, ki_array, 'o')
#ax4.plot(kj_inv_fit, ki_fit, 'r')
ax4.plot(kj_inv_fit, ki_fit, 'r')
ax4.set_xlabel('1/(joystick gain) =  (1/kj)')
ax4.set_ylabel('integral gain (ki)')
ax4.grid(True)

fig5, ax5 = plt.subplots(1,1)
ax5.plot(kj_array, ki_array, 'o')
#ax5.plot(kj_median, ki_median, 'r')
#ax5.plot(kj_fit, ki_fit, 'r')
ax5.plot(kj_fit, km_lstsq/kj_fit, 'r')
ax5.set_xlabel('joystick gain (kj)')
ax5.set_ylabel('integral gain (ki)')
ax5.grid(True)

fig6, ax6 = plt.subplots(1,1)
ax6.plot(kj_array, ki_array*kj_array,'o')
ax6.set_xlabel('joystick gain (kj)')
ax6.set_ylabel('total gain (ki*kj)')
ax6.set_ylim([0, 1.5])
ax6.grid(True)

print(f'km_fit: {km_fit}')
print(f'km_median: {km_median}')
print(f'km_lstsq: {km_lstsq}')
